...
- **`files[]`** — the blast radius: target file + every importer (transitively) + every shared type/helper. Grep `from '.*X'` and walk the import chain. The `scope-refresh` hook auto-records file-edit-tool edits, and `scope-git-sweep` catches Shell-written files (heredocs, redirections, build outputs) via a postToolUse git diff — declare the expected radius at Step 0.
