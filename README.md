...
When an implementation finishes, a stop hook fires exactly one final review pass over everything that changed — then stops. Every bound is enforced twice: in the script and in `hooks.json`.
...
windows/          PowerShell hooks (pwsh) — install on Windows machines
  ...
  hooks/          the seven scripts + the three prompt files
...
| Every turn | `postToolUse` | Drains the conversation's pending feedback file into `additional_context`. One-shot, keyed by conversation id. |
