...
5. **Intent anchor (persistent contract nudge)** at `postToolUse` — `intent-anchor` re-fires whenever new files are edited and `.scope.json` has `[DRAFT]`/empty `intent` or default-seed `acceptance`. The per-cid flag stores `filesCount:nudgeCount`; if no new files since, it stays silent. Capped at 3 nudges per session. Once both fields are filled, it goes silent permanently for that conversation. Emits an `INTENT ANCHOR` reminder so the agent fills the agent-owned half of the contract. The hook never writes those fields; it just surfaces the gap.
...
- **State lives under `$HOME`**, in `~/.cursor/.hooks-pending/`, keyed by conversation id: `reviewed-<cid>.flag` (the verify-revise brake for `final-review`, stores changed-file count), `intent-anchored-<cid>.flag` (stores `filesCount:nudgeCount`), and a transient `scope-<cid>.txt` stash (written by `scope-refresh`, deleted by `scope-drain` on the next tool boundary). Stale state older than 7 days gets swept on every stop. The only repo file is `.scope.json`.
...
- **Verify-revise-reverify.** The stop hook stores the changed-file count in a per-conversation flag before emitting its follow-up. If the agent revises (diff changes), the review re-fires. If the agent accepts (same diff), the loop ends. Bounded by `loop_limit: 3`.
